...
def _read_proc_sample(proc_root: Path, pid: int, *, clk_tck: int, page_size: int) -> ProcSample | None:
...
def render(
    *,
    ...
    lines.append("# HELP node_textfile_top_process_cpu_percent Top processes by CPU usage over the last interval.")
    lines.append("# TYPE node_textfile_top_process_cpu_percent gauge")
    for rank, sample, cpu_percent in top_cpu:
        lines.append(
            'node_textfile_top_process_cpu_percent{rank="%d",pid="%d",comm="%s"} %g'
            % (rank, sample.pid, _escape_label_value(sample.comm), cpu_percent)
        )

    lines.append("# HELP node_textfile_top_process_rss_bytes Top processes by resident memory (RSS).")
    lines.append("# TYPE node_textfile_top_process_rss_bytes gauge")
    for rank, sample in top_rss:
        lines.append(
            'node_textfile_top_process_rss_bytes{rank="%d",pid="%d",comm="%s"} %d'
            % (rank, sample.pid, _escape_label_value(sample.comm), sample.rss_bytes)
        )

    if mem_total_bytes and mem_total_bytes > 0:
        lines.append("# HELP node_textfile_top_process_mem_percent Top processes by RSS as percent of total RAM.")
        lines.append("# TYPE node_textfile_top_process_mem_percent gauge")
        for rank, sample in top_rss:
            mem_percent = (sample.rss_bytes / mem_total_bytes) * 100.0
            lines.append(
                'node_textfile_top_process_mem_percent{rank="%d",pid="%d",comm="%s"} %g'
                % (rank, sample.pid, _escape_label_value(sample.comm), mem_percent)
            )
...
def main() -> int:
    ...
    def run_once(interval_seconds: float) -> None:
        nonlocal prev_ts, prev_cpu
        now_ts = time.monotonic()
        dt = max(0.001, now_ts - prev_ts) if interval_seconds > 0 else max(0.001, interval_seconds)
        ...
        try:
            for child in proc_root.iterdir():
                ...
                if not child.name.isdigit():
                    continue
                pid = int(child.name)
                sample = _read_proc_sample(proc_root, pid, clk_tck=clk_tck, page_size=page_size)
        ...
        # Update history
        prev_cpu = {s.pid: s.cpu_total_seconds for s in samples}
        prev_ts = now_ts
        ...
        mem_total_bytes = _read_mem_total_bytes(proc_root)
    ...
    while True:
        run_once(args.interval_seconds)
