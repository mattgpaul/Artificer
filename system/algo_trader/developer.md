...
# Or set inline
REDIS_HOST=custom-redis-server bazel run //system/algo_trader/service:live_market_service
```
...
**Standalone**:
```bash
LOG_LEVEL=DEBUG bazel run //system/algo_trader/service:live_market_service
```
...
config = RedisConfig(port="not-a-number")  # ❌ TypeError
```
