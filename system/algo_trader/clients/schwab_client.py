"""Algo-trader Schwab client wrapper.

This module provides a system-level client that handles token storage/refresh and
delegates HTTP interactions to the infrastructure Schwab client.
"""
...
DEFAULT_TOKEN_FILE_PATH = "/home/matthew/Artificer/infrastructure/clients/schwab/schwab_tokens.json"
...
class AlgoTraderSchwabClient:
    ...
    def __init__(
        ...
        token_file_path: str = DEFAULT_TOKEN_FILE_PATH,
    ):
        ...
        self._auth_attempted_this_session = (
            False  # Track if we've already tried to authenticate this session
        )

    def _read_refresh_token_from_file(self) -> str | None:
    ...
    def _maybe_authenticate_once(self) -> str | None:
        """Attempt interactive OAuth2 once per session and return access token if stored."""
        if self._auth_attempted_this_session:
            return None
    ...
    def _get_valid_token(self) -> str | None:
        ...
        if not refresh_token:
            self.logger.warning(
                "No refresh token found in token file, triggering OAuth2 authentication"
            )
            token = self._maybe_authenticate_once()
            if token is None:
                self.logger.error(
                    "No refresh token available and authentication already attempted this session"
                )
            return token
        ...
        if token is None:
            self.logger.error(
                "Token refresh failed and authentication already attempted this session"
            )
        return token
    ...
    def _refresh_token(self, refresh_token: str) -> str | None:
        ...
        try:
            ...
            if new_refresh_token:
                ...
                self.logger.info(
                    "Token refresh successful - updated both access and refresh tokens"
                )
            else:
                self.logger.warning(
                    "Token refresh successful but no new refresh token returned - "
                    "this is unexpected!"
                )
    ...
    def get_price_history(
        ...
        try:
            ...
            response = requests.get(url, headers=headers, params={**params, "symbol": symbol})
            ...
            if response.status_code == 200:
                ...
                self.logger.error(
                    f"Price history request failed: {response.status_code} - {response.text}"
                )
                return None
        except Exception as e:
