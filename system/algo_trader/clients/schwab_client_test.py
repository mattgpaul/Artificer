...
class TestAlgoTraderSchwabClientUnit:
    ...
    @patch("os.makedirs")
    @patch("os.chmod")
    def test_authenticate_success(self, *mocks):
        """Test successful authentication flow."""
        _mock_chmod, _mock_makedirs, _mock_open, mock_input, _mock_redis, _mock_pool = mocks
        redis_client = AlgoTraderRedisClient()
    ...
    def test_refresh_token_success(self, *mocks):
        """Test successful token refresh updates BOTH access and refresh tokens."""
        _mock_exists, _mock_chmod, _mock_makedirs, mock_file, _mock_redis, _mock_pool = mocks
        redis_client = AlgoTraderRedisClient()
        mock_redis_instance = MagicMock()
        mock_redis_instance.set.return_value = True
        redis_client.client = mock_redis_instance
    ...
    def test_refresh_token_failure(self, mock_redis, mock_pool):
        ...
        mock_schwab = MagicMock()
        mock_schwab.refresh_access_token.side_effect = Exception(
    ...
    @patch("os.chmod")
    @patch("os.path.exists", return_value=False)
    def test_write_refresh_token_to_file(self, *mocks):
        """Test writing refresh token to file."""
        _mock_exists, mock_chmod, _mock_makedirs, mock_file, _mock_redis, _mock_pool = mocks
        redis_client = AlgoTraderRedisClient()
        client = AlgoTraderSchwabClient(redis_client)
