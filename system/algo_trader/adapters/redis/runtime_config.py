...
class AlgoTraderRuntimeConfigStore(BaseRedisClient):
    ...
    def get_watchlist(self, engine_id: str, limit: int = 200) -> list[str]:
        ...
        items = {x.strip().upper() for x in self.smembers(f"{engine_id}:watchlist") if str(x).strip()}
        out = sorted(items)
        return out[:limit]

    def set_watchlist(self, engine_id: str, symbols: Iterable[str], ttl_seconds: int | None = None) -> None:
    ...
    def add_to_watchlist(self, engine_id: str, symbols: Iterable[str], ttl_seconds: int | None = None) -> None:
