...
def mock_logger():
    ...
    with patch("algo_trader.infra.schwab.schwab_client.get_logger") as mock_logger_func:
...
def mock_account_broker():
    ...
    with patch("algo_trader.infra.schwab.schwab_client.AccountBroker") as mock_broker_class:
...
def mock_token_manager_requests():
    ...
    with patch("algo_trader.infra.schwab.auth.token_manager.requests") as mock_requests:
        yield mock_requests
...
def mock_oauth2_requests():
    ...
    with patch("algo_trader.infra.schwab.auth.oauth2.requests") as mock_requests:
        yield mock_requests
...
def mock_oauth2_input():
    ...
    with patch("algo_trader.infra.schwab.auth.oauth2.input") as mock_input:
        yield mock_input
...
def mock_oauth2_print():
    ...
    with patch("algo_trader.infra.schwab.auth.oauth2.print") as mock_print:
        yield mock_print
...
def mock_schwab_client_requests():
    ...
    with patch("algo_trader.infra.schwab.schwab_client.requests") as mock_requests:
        yield mock_requests
...
def mock_token_manager_time():
    ...
    with patch("algo_trader.infra.schwab.auth.token_manager.time.sleep") as mock_sleep:
        yield mock_sleep
