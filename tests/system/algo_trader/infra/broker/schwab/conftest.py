...
def token_manager_mock() -> dict[str, Any]:
    ...
    """
    with patch("system.algo_trader.infra.broker.schwab.schwab_client.TokenManager") as MockTM:
        instance = MagicMock()
...
def requests_mock() -> MagicMock:
    ...
    """
    with patch("system.algo_trader.infra.broker.schwab.schwab_client.requests") as mock_requests:
...
def account_handler():
    ...
    """
    from system.algo_trader.infra.broker.schwab.account_handler import AccountHandler
...
def market_handler():
    ...
    from system.algo_trader.infra.broker.schwab.market_handler import MarketHandler
...
def token_manager_fixtures():
    ...
    """
    with (
        patch("system.algo_trader.infra.broker.schwab.token_manager.AccountBroker") as BrokerCls,
        patch("system.algo_trader.infra.broker.schwab.token_manager.requests") as requests_mod,
        patch("system.algo_trader.infra.broker.schwab.token_manager.time") as time_mod,
    ):
        broker = MagicMock()
...
def oauth2_requests_mock() -> MagicMock:
    ...
    with patch("system.algo_trader.infra.broker.schwab.oauth2_handler.requests") as mock_requests:
        yield mock_requests
