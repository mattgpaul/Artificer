...
class TestOHLCVArgumentHandlerInfluxDBIntegration:
    ...
    def test_execute_writes_to_influxdb_successfully(self, mock_dependencies):
        """Test execute writes OHLCV data to InfluxDB successfully."""
        with (
            patch(
                "system.algo_trader.datasource.populate.ohlcv_args.MarketHandler"
            ) as mock_market_handler_class,
            ...
            patch(
                "system.algo_trader.datasource.populate.ohlcv_args.MarketDataInflux"
            ...
            mock_thread_manager.config.max_threads = 10
            mock_thread_manager.get_active_thread_count.return_value = 0
            mock_thread_manager.cleanup_dead_threads.return_value = 1
            mock_thread_manager.get_results_summary.return_value = {
                "successful": 1,
                "failed": 0,
                "running": 0,
            ...
            # Verify InfluxDB write was called (called within thread, so check it was set up)
    ...
    def test_execute_handles_influxdb_write_failure(self, mock_dependencies):
        ...
        with (
            ...
            mock_market_handler.get_price_history.return_value = {
                "symbol": "AAPL",
                "candles": [{"datetime": 1234567890, "close": 150.0}],
            }
            mock_market_handler_class.return_value = mock_market_handler
            ...
            # Mock MarketDataInflux with write failure
            mock_influx_client = Mock()
            mock_influx_client.database = "algo-trader-database"
            mock_influx_client.write.return_value = False  # Simulate write failure
            mock_influx_class.return_value = mock_influx_client
    ...
    def test_execute_no_longer_prints_statistics(self, mock_dependencies):
        """Test execute does not print ticker statistics to console."""
        with (
            patch(
                "system.algo_trader.datasource.populate.ohlcv_args.MarketHandler"
            ) as mock_market_handler_class,
            ...
            # Verify no statistics prints (should only have summary prints)
            assert not any("Statistics" in str(call) for call in print_calls)
            assert not any("Price Range:" in str(call) for call in print_calls)
            assert not any("Average Close:" in str(call) for call in print_calls)

            # Verify summary prints are still present
            assert any("OHLCV Data Population Summary" in str(call) for call in print_calls)
