...
class TestMarketDataInfluxInitialization:
    ...
    def test_initialization_default_database(self, mock_influx_dependencies):
    ...
    def test_initialization_custom_database(self, mock_influx_dependencies):
    ...
    def test_initialization_uses_market_write_config_by_default(self, mock_influx_dependencies):
    ...
    def test_initialization_creates_logger(self, mock_influx_dependencies):
        """Test that initialization creates a logger with class name."""
        MarketDataInflux()

        mock_influx_dependencies["logger"].assert_called_with("MarketDataInflux")


class TestMarketDataInfluxFormatStockData:
    """Test stock data formatting."""

    def test_format_stock_data_success(self, mock_influx_dependencies):
    ...
    def test_format_stock_data_datetime_conversion(self, mock_influx_dependencies):
    ...
    def test_format_stock_data_logs_debug(self, mock_influx_dependencies):
    ...
    def test_format_stock_data_preserves_all_columns(self, mock_influx_dependencies):
...
class TestMarketDataInfluxWrite:
    ...
    def test_write_stock_data_success(self, mock_influx_dependencies):
    ...
    def test_write_increments_pending_counter(self, mock_influx_dependencies):
    ...
    def test_write_adds_ticker_tag(self, mock_influx_dependencies):
    ...
    def test_write_multiple_tickers(self, mock_influx_dependencies):
...
class TestMarketDataInfluxQuery:
    ...
    def test_query_success(self, mock_influx_dependencies):
        ...
        assert isinstance(result, pd.DataFrame)
        assert len(result) == 2
        mock_influx_dependencies["client"].query.assert_called_once_with(
    ...
    def test_query_failure_returns_false(self, mock_influx_dependencies):
        ...
        mock_influx_dependencies["client"].query.side_effect = Exception("Query failed")
    ...
    def test_query_failure_logs_error(self, mock_influx_dependencies):
        """Test that query failure logs error message."""
        mock_influx_dependencies["client"].query.side_effect = Exception("Connection timeout")
    ...
    def test_query_with_complex_sql(self, mock_influx_dependencies):
    ...
    def test_query_empty_result(self, mock_influx_dependencies):
    ...
    def test_query_uses_sql_language(self, mock_influx_dependencies):
        ...
        call_args = mock_influx_dependencies["client"].query.call_args
        assert call_args[1]["language"] == "sql"

    def test_query_uses_pandas_mode(self, mock_influx_dependencies):
...
class TestMarketDataInfluxIntegration:
    ...
    def test_write_then_query_workflow(self, mock_influx_dependencies):
    ...
    def test_batch_write_multiple_tickers(self, mock_influx_dependencies):
        ...
        assert mock_influx_dependencies["client"].write.call_count == len(tickers)

    def test_error_recovery_continues_after_failed_write(self, mock_influx_dependencies):
        """Test that client can continue after a failed write."""
        # First write fails, second succeeds
        mock_influx_dependencies["client"].write.side_effect = [
