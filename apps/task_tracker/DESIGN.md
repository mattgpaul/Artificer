...
- **Testing convention — none exists yet; do not invent one.** `tests/` is currently empty and the repo is young, so there is no established layout to mirror into. Where task_tracker's tests live is a decision to make when tests are first written, not a design assumption to bake in now. (Earlier draft guessed `tests/system/task_tracker/`; withdrawn — that convention does not exist.)
